...
${error.stack}`);
  ...
  async setupDatabase() {
    if (!import_config_loader.Config.usesqlite) return false;
  ...
  async write(roomid, entry, overrideID) {
    if (!import_config_loader.Config.usesqlite || !import_config_loader.Config.usesqlitemodlog) return;
  ...
  async writeSQL(entries) {
    if (!import_config_loader.Config.usesqlite) return;
  ...
  async getGlobalPunishments(user, days = 30) {
    if (!import_config_loader.Config.usesqlite || !import_config_loader.Config.usesqlitemodlog) return null;
